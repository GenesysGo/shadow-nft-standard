...
export class TooManyCreators extends Error {
  ...
  readonly msg =
    "Can only provide up to 4 creators to ssm (because ssm is one)";

  constructor(readonly logs?: string[]) {
    super(
      "6003: Can only provide up to 4 creators to ssm (because ssm is one)"
    );
  }
}
...
export function minterFromCode(
  code: number,
  logs?: string[]
): CustomError | null {
  switch (code) {
